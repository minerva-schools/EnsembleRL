...
class CartPoleQAgent():
    def __init__(self, buckets=(3, 3, 6, 12), num_episodes=500, min_lr=0.1, min_epsilon=0.1, discount=1.0, decay=25):
    ...
    def get_action(self, state, e):
        """This method assumes the state fed hasn't been discretized."""
        obs = discretize_state(state)
        max_action = np.argmax(self.Q_table[obs])
        epsilon = self.get_epsilon(e)
        print(epsilon)
        action_vector = np.zeros(2)
        action_vector[max_action-1] = epsilon/2.0
        action_vector[max_action] = 1-action_vector[max_action-1]
